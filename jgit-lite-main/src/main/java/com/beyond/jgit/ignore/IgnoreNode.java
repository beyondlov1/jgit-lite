...
public class IgnoreNode {
		...
		while ((txt = br.readLine()) != null) {
			if (txt.length() > 0 && !txt.startsWith("#") && !txt.equals("/")) { //$NON-NLS-1$ //$NON-NLS-2$
				...
				try {
					...
					if (sourceName != null) {
						LOG.error("badIgnorePatternFull error", e);
					} else {
						LOG.error("badIgnorePattern error", e);
					}
				}
				if (!rule.isEmpty()) {
					rules.add(rule);
	 ...
	 */
	public @Nullable
	Boolean checkIgnored(String entryPath,
						 boolean isDirectory) {
	...
	public static boolean isIgnored(IgnoreNode ignoreNode, File file, File root){
		if (!file.exists()){
		...
		final String relativePath = PathUtils.getRelativePath(root.getPath(), file.getAbsolutePath());
		final MatchResult matchResult = ignoreNode.isIgnored(relativePath, file.isDirectory());
		if (matchResult == MatchResult.CHECK_PARENT){
			return isIgnored(ignoreNode, file.getParentFile(), root);
		}
	...
	public static IgnoreNode load(String ignoreFilePath){
		IgnoreNode ignoreNode = new IgnoreNode();
		try (FileInputStream ignoreFile = new FileInputStream(ignoreFilePath)){
			ignoreNode.parse(ignoreFile);
		} catch (IOException e) {
			LOG.error("load ignore error",e);
			return null;
		}
		return ignoreNode;
	}
	...
	public static void main(String[] args) throws IOException {
		IgnoreNode ignoreNode = new IgnoreNode();
		try (FileInputStream ignoreFile = new FileInputStream("/home/beyond/github/jgit-lite/.gitignore")){
			ignoreNode.parse(ignoreFile);
			final List<FastIgnoreRule> rules = ignoreNode.getRules();
			for (FastIgnoreRule rule : rules) {
				System.out.println(rule);
			}
			final boolean ignored = isIgnored(ignoreNode, new File("/home/beyond/github/jgit-lite/jgit-lite-delta/src/main/java/com/beyond/delta/entity"), new File("/home/beyond/github/jgit-lite"));
